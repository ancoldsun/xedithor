...
{
    // create interface
    //std::cout<<" 1==dir: "<<m_exportOutSrcDir.toStdString().c_str()<<std::endl;
    //QFileInfo infof(m_exportOutSrcDir);
    //QDir infod= infof.dir();
    //QString strDir = infod.absolutePath();
    m_sprName = m_sprName.toUpper();
    QString s=m_exportOutSrcDir+"/"+m_sprName+".java";

    //std::cout<<" 2==dir: "<<s.toStdString().c_str()<<std::endl;
    QFile file(s);
